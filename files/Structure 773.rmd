...
TEST_773 <- filter(xl_NewDenC,`Structure-ID` == 773)
...
Below is the code responsible for generating the correlation matrix of Structure-ID: 773.
...
nu_TEST_773 <- TEST_773 %>%   select(-c(`Setd5-37`, `Adcy9-83`, `Col6a1-290`, `Dbh-326`, `Erdr1-378`, `Enc1-390`, `Hars-554`, `Kitl-616`, `L1cam-620`, `Pcsk1n-777`, `Pkia-805`, `Prkcz-841`, `Sst-1001`, `Ywhag-1124`, `Nr1d2-1245`, `Esrra-1351`, `Pex14-1397`, `Snhg7-1968`, `Ankrd33-2006`, `Adrm1-2019`, `Crat-2119`, `Fbln1-2205`, `Ndufs1-2421`, `Nfkbie-2425`, `Traf7-2527`, `Aurka-2643`, `Mapk3-32576`, `Pim2-32583`, `Limk2-61002`, `Bcap29-104826`, `Gabpb1-121098`, `Ptbp1-356857`, `Gm10413-73929885`, `Fxyd5-74819694`))
```

```{r}
#Creating correlation matrix again
nu_correlation_773 <- cor(nu_TEST_773)
```
...
Below is my attempt at screening through the produced correlation matrix (nu_correlation_773) and returning specific gene pairs whose correlation coefficient values are equal to or greater than 90%.
...
highlyCorrelated90 <- findCorrelation(nu_correlation_773, cutoff = .90, names = TRUE)
...
correlated_pairs90 <- which(abs(nu_correlation_773) > 0.9, arr.ind = TRUE)
...
Below is my attempt at screening through the produced correlation matrix (nu_correlation_773) and returning specific gene pairs whose correlation coefficient values are equal to or greater than 80%.

```{r}
# METHOD 1 
#Screening through matrix and returning columns (genes) that have 80% correlation or greater
highlyCorrelated80 <- findCorrelation(nu_correlation_773, cutoff = .80, names = TRUE)
...
correlated_pairs80 <- which(abs(nu_correlation_773) > 0.8, arr.ind = TRUE)
...
Below is my attempt at screening through the produced correlation matrix (nu_correlation_773) and returning specific gene pairs whose correlation coefficient values are equal to or greater than 70%.

```{r}
# METHOD 1 
#Screening through matrix and returning columns (genes) that have 70% correlation or greater
highlyCorrelated70 <- findCorrelation(nu_correlation_773, cutoff = 0.70, names = TRUE)
...
correlated_pairs70 <- which(abs(nu_correlation_773) > 0.7, arr.ind = TRUE)
